...
public class UserUpdateController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		RequestDispatcher dispatcher = req.getRequestDispatcher("/view/admin/UpdateUser.jsp");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 * (FileItem item : items) { if (item.getFieldName().equals("id")) {
		 * user.setId(Integer.parseInt(item.getString())); } else if
		 * (item.getFieldName().equals("email")) { user.setEmail(item.getString()); }
		 ...
		 * (item.getFieldName().equals("avatar")) { if (item.getSize() > 0) {// neu co
		 * file d final String dir = "F:\\upload"; String originalFileName =
		 ...
		 * 
		 * user.setAvatar(fileName); } else { user.setAvatar(null); } } }
		 * 
		 * userService.edit(user);
		 * 
		 * resp.sendRedirect(req.getContextPath() + "/admin/user/list"); ---thay doi
		 * phan nay
		 */
		try {
